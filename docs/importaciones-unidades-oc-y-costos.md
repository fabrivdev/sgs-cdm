...
Al adjuntar o reemplazar la factura del proveedor, la app lee automáticamente número, fecha, moneda y total. Los PDF con texto se procesan localmente; las imágenes usan el lector documental existente. Los campos detectados se guardan en la unidad y la extracción queda asociada al documento. Una lectura parcial conserva lo detectado y pide revisar únicamente lo faltante; una falla de lectura no impide adjuntar el archivo. Registrar arribo se ofrece solo en Planificado o En tránsito: después de guardar una ATA, Arribado queda a la espera de conciliación con Stock/Parque sin repetir la acción. El detalle permanece abierto y recarga la misma unidad tras guardar para reflejar de inmediato la nueva etapa o los datos de factura.
