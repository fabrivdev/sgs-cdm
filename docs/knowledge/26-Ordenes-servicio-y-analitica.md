...
Entregar completo para copiar/pegar el archivo `supabase/migrations/20260925150000_service_orders_section.sql` en Lovable Cloud → SQL editor. Crea la sección y copia una sola vez las asignaciones del Dashboard (sin conceder nuevos roles); desactiva la sección anterior y adapta la lectura restringida de la meta. Repetirlo no repone permisos revocados. No modifica órdenes ni facturas y no ha sido ejecutado por el agente: un push no aplica SQL ni valida producción.
