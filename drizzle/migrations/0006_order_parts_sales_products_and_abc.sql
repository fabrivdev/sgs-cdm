...
BEGIN
  ...
  producto_marcas AS (
    ...
    SELECT CASE WHEN p_vista='clientes' THEN cliente WHEN p_vista='vendedores' THEN coalesce(vendedor,'Sin vendedor') ELSE coalesce(nullif(codigo,''),nullif(codigo_fabricante,''),descripcion) END id,
      ...
      CASE WHEN p_vista='repuestos' THEN coalesce(nullif(codigo,''),nullif(codigo_fabricante,'')) END codigo,
    ...
    FROM actual WHERE p_vista IN('clientes','repuestos','vendedores') GROUP BY 1,2,3
  ), agrupados_marca AS (
