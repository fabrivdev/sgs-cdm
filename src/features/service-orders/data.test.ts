...
describe("operational source loader", () => {
  ...
  it("returns operational data without starting or awaiting financial work", async () => {
    const { client, calls } = clientFixture({ failure: "service_orders_billing_v1" });
    const result = await loadOperationsData(client, "2026-09-01", "2026-09-30");
    expect(result.data.ordenesServicio).toHaveLength(3);
    expect(result.data.billing).toBeUndefined();
    expect(calls.find(c => c.table === "service_orders_billing_v1")).toBeUndefined();
  });
