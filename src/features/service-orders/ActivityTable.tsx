...
export function ActivityTable({ model }: { model: OperationsModel }) {
  ...
  return <section className="min-w-0 space-y-2 border-t pt-3"><h2 className="text-[13px] font-semibold">Actividad por técnico</h2>
    <CompactListTable rows={table.ordered} columns={columns.slice(0, 5)} mobileColumns={[
      { ...columns[0], width: "w-[72%]", render: r => <MobileRecord primary={r.tecnico} secondary={r.cliente || r.trabajo} context={`${r.fecha} · ${r.ref || r.sucursal}`} /> },
      { ...columns[4], width: "w-[28%]" },
    ]} id={r => r.key} label="Actividad por técnico" heading={table.heading} sort={table.sort} status={!rows.length ? "Sin actividad en la matriz." : undefined} />
  </section>;
}
