...
export interface ResultadoImportacionHistorialLegacy {
  lote_id: string;
  filas: number;
  confirmadas: number;
...
function numeroCelda(value: unknown) {
  if (typeof value === "number") return Number.isFinite(value) ? value : 0;
  const raw = textoCelda(value).replace(/\s/g, "");
  if (!raw) return 0;
...
function fechaCelda(value: unknown) {
...
export async function importarHistorialLegacy(
  file: File,
  marca: MarcaSugerencia,
  onProgress?: (loaded: number, total: number) => void,
) {
  ...
  const sheetName = workbook.SheetNames.find((name) => name.trim().toUpperCase() === "VENTAS");
  if (!sheetName) throw new Error("El archivo no contiene la hoja VENTAS");

  const rawRows = XLSX.utils.sheet_to_json<Record<string, unknown>>(workbook.Sheets[sheetName], {
    defval: null,
    raw: true,
  });
  const fabricantesPorCodigo = new Map<string, string>();
  for (const row of rawRows) {
    const codigo = textoCelda(row["codigo interno"] ?? row["C\u00f3digo interno"] ?? row["C\u00f3d. Merc"]);
    const fabricante = textoCelda(row["C\u00f3d. Fabr."] ?? row["Cod. Fabr."] ?? row["C\u00f3digo fabricante"]);
    if (codigo && fabricante) fabricantesPorCodigo.set(codigo, fabricante);
  }

  const detailSheetName = workbook.SheetNames.find((name) => name.trim().toUpperCase() === "SALIDAS SKUS");
  const detailRows = detailSheetName
    ? XLSX.utils.sheet_to_json<Record<string, unknown>>(workbook.Sheets[detailSheetName], { defval: null, raw: true })
    : [];
  const sourceRows = detailRows.length > 0
    ? detailRows.flatMap((row) => {
      const codigo = textoCelda(row["C\u00f3d. Mercader\u00eda"]);
      if (!fabricantesPorCodigo.has(codigo)) return [];
      return [{
        ...row,
        "codigo interno": codigo,
        "C\u00f3d. Fabr.": fabricantesPorCodigo.get(codigo),
        Mercaderia: row["Descripci\u00f3n"],
        fecha: row.Fecha,
        "Cant.Unit": row["Cantidad Real"],
        "Total Ventas": row["Total US$"],
      }];
    })
    : rawRows;
  const rows = sourceRows.flatMap((row, index) => {
    const codigoLegacy = textoCelda(row["codigo interno"] ?? row["Código interno"] ?? row["Cód. Mercadería"]);
    const codigoFabricante = textoCelda(row["Cód. Fabr."] ?? row["Cod. Fabr."] ?? row["Código fabricante"]);
    const descripcion = textoCelda(row.Mercadería ?? row.Mercaderia ?? row.Descripción);
    const fecha = fechaCelda(row.fecha ?? row.Fecha);
    const cantidad = numeroCelda(row["Cant.Unit"] ?? row.Cantidad);
    const importeUsd = numeroCelda(row["Total Ventas"] ?? row.Total);
    const documento = textoCelda(row["Nro. Documento"]);
    if (!codigoLegacy || !fecha || (cantidad === 0 && importeUsd === 0)) return [];
    return [{
      linea_clave: `${fecha}|${documento}|${codigoLegacy}|${codigoFabricante}|${index + 2}`,
      fecha,
      documento: documento || `VENTAS-${fecha.slice(0, 7)}-${codigoLegacy}`,
      codigo_legacy: codigoLegacy,
      codigo_fabricante: codigoFabricante || null,
      descripcion: descripcion || `Producto historico ${codigoLegacy}`,
      cantidad,
      importe_usd: importeUsd,
    }];
  });
  if (rows.length === 0) throw new Error("El archivo no contiene movimientos utilizables para la marca seleccionada");

  const start = await (supabase.rpc as any)("repuestos_iniciar_importacion_historial_legacy", {
    p_marca: marca,
    p_archivo_nombre: file.name,
  });
  if (start.error) throw start.error;
  const loteId = start.data as string;
  const chunkSize = 500;
  for (let offset = 0; offset < rows.length; offset += chunkSize) {
    const chunk = rows.slice(offset, offset + chunkSize);
    const result = await (supabase.rpc as any)("repuestos_importar_historial_legacy_lote", {
      p_lote_id: loteId,
      p_filas: chunk,
    });
  ...
  const finish = await (supabase.rpc as any)("repuestos_finalizar_importacion_historial_legacy", {
    p_lote_id: loteId,
  });
  if (finish.error) throw finish.error;
  return finish.data as ResultadoImportacionHistorialLegacy;
}
