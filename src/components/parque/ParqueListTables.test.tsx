...
describe("Parque and imports compact lists",()=>{
  it("orders preserves billing, delivery, money and the original selection",()=>{
    const row={id:"U1",operacion_id:"OP1",np_numero:"000101",np_fecha:"2026-09-17",cliente_nombre:"CLIENTE CON NOMBRE MUY LARGO",comercial:null,marca:"CLAAS",producto:"COSECHADORAS",modelo:machine.modelo_tipo,cantidad:1,condicion:"NUEVA",abastecimiento:"STOCK",estado_fuente:"DISPONIBLE",estado_operacion:"DISPONIBLE",chasis:machine.serie,estado_disponibilidad:"DISPONIBLE",disponibilidad_detalle:null,estado_importacion_fuente:null,eta:null,ata:null,proveedor:null,factura_venta:null,factura_fecha:null,costo_producto:null,valor_venta:111234.56,moneda_valor:"USD",observaciones:null,actualizado_en:"2026-09-18",es_historico:false};
    const select=vi.fn();setup(<OrdersTable rows={[row]} heading={key=>key} onSelect={select} entregaByUnitId={new Map([[row.id,{estado:"DISPONIBLE",chasis:machine.serie}]])} estadoByOperacionId={new Map([[row.operacion_id,"DISPONIBLE"]])} stockChasisSet={new Set([machine.serie])}/>);
