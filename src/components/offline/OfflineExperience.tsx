  ...
  return (
    <section className="offline-experience" role="alert" aria-live="assertive">
      ...
      <div className="offline-heading">
        <span className="offline-eyebrow">SIG · Sistema Integrado de Gestión</span>
        <h1>Conexión interrumpida</h1>
        <p>La aplicación se reanudará automáticamente cuando vuelva internet.</p>
      </div>
      <div className="offline-lanyard-stage" aria-hidden="true">
        {showMotion ? (
          <Suspense fallback={<StaticCredential />}>
            <Lanyard position={[0, 0, 13]} gravity={[0, -40, 0]} fov={22} frontImage="/offline-card.svg" backImage="/offline-card.svg" imageFit="cover" lanyardImage="/offline-lanyard.svg" logoImage="/sig-cdm-logo.png" lanyardWidth={1.15} />
          </Suspense>
        ) : (
          <StaticCredential />
        )}
