    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button type="button" variant="outline" size="icon" aria-label="Acciones de la sección" title="Acciones de la sección"
          className={cn("h-11 w-11 shrink-0 max-sm:border-0 max-sm:bg-transparent max-sm:shadow-none sm:h-8 sm:w-8", className)} disabled={busy || pending || options.every(option => option.disabled)}>
