...
function Heads({ labels }: { labels: string[] }) {
  return <thead className={`text-muted-foreground ${salesHeader}`}><tr>{labels.map(label => <th key={label} className={cn("sticky top-0 z-10 bg-muted/60", salesColumnClass(label))}>{label}</th>)}</tr></thead>;
}
