...
export function ServiciosClientes({ desde, hasta, sucursal, buscar, tipoTiempo, marca = "", tipoMaquina = "" }: { desde: string; hasta: string; sucursal: string; buscar: string; tipoTiempo: string; marca?: string; tipoMaquina?: string }) {
  ...
  const grid = "grid-cols-[minmax(220px,1fr)_60px_75px_110px_105px_105px_95px_95px_110px_95px]";
  return <><label className="mt-3 flex items-center gap-2 text-[11px]">Agrupar por<select aria-label="Agrupar clientes por" value={perspective} onChange={e=>setPerspective(e.target.value as "propietario" | "cliente")} className="h-8 rounded-md border bg-background px-2 text-[11px]"><option value="propietario">Propietarios actuales</option><option value="cliente">Clientes facturados</option></select></label><div className="mt-2 overflow-hidden rounded-md border"><div className="overflow-x-auto"><div className="min-w-[1120px]">
    <TableScroll rows={rows.length}>
    <div className={`grid ${grid} ${scrollHead} items-center bg-muted/60 px-3 py-2 text-[11px] font-medium text-muted-foreground ${salesHeader}`}><div>{perspective === "propietario" ? "Cliente" : "Cliente facturado"}</div><div className="whitespace-nowrap text-center">OS</div><div className="whitespace-nowrap text-center">Facturas</div><div className="whitespace-nowrap text-center">Notas de crédito</div><div className="whitespace-nowrap text-right">Mano de Obra</div><div className="whitespace-nowrap text-right">Kilometraje</div><div className="whitespace-nowrap text-right">Repuestos</div><div className="whitespace-nowrap text-right">Terceros</div><div className="whitespace-nowrap text-right">Neto</div><div className="whitespace-nowrap text-right">Participación</div></div>
    {!rows.length ? <div className="py-12 text-center text-[12px] text-muted-foreground">No hay clientes para el período.</div> : rows.map((row) => <div key={row.cliente} className={`grid ${grid} items-center border-t px-3 py-1.5 text-[12px]`}><div className="truncate font-medium">{row.cliente}</div><div className="text-center tabular-nums">{integer.format(row.os.size)}</div><div className="text-center tabular-nums">{integer.format(row.facturas.size)}</div><div className="text-center tabular-nums text-muted-foreground">{integer.format(row.notas.size)}</div><div className="text-right tabular-nums text-muted-foreground">{usd.format(row.mo)}</div><div className="text-right tabular-nums text-muted-foreground">{usd.format(row.km)}</div><div className="text-right tabular-nums text-muted-foreground">{usd.format(row.repuestos)}</div><div className="text-right tabular-nums text-muted-foreground">{usd.format(row.terceros)}</div><div className="text-right font-semibold tabular-nums">{usd.format(row.total)}</div><div className="text-right tabular-nums text-muted-foreground">{totalPeriodo ? `${Math.round((row.total / totalPeriodo) * 100)}%` : "—"}</div></div>)}
    </TableScroll>
