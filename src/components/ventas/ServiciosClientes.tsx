...
export function ServiciosClientes({ desde, hasta, sucursal, buscar, tipoTiempo }: { desde: string; hasta: string; sucursal: string; buscar: string; tipoTiempo: string }) {
  ...
  const rows = useMemo(() => {
    type Summary = { cliente: string; total: number; previous: number; facturas: Set<string>; os: Set<string>; mo: number; km: number; repuestos: number; terceros: number };
    const map = new Map<string, Summary>();
    const get = (name: string) => { const key = name || "Sin cliente"; const value = map.get(key) ?? { cliente: key, total: 0, previous: 0, facturas: new Set<string>(), os: new Set<string>(), mo: 0, km: 0, repuestos: 0, terceros: 0 }; map.set(key, value); return value; };
  ...
  return <div className="mt-3 overflow-x-auto rounded-md border"><div className="min-w-[1080px]">
    <div className="grid grid-cols-[minmax(230px,1fr)_70px_70px_repeat(4,110px)_130px_105px] bg-muted/60 px-3 py-2 text-[11px] font-medium text-muted-foreground"><div>Cliente</div><div className="text-right">OS</div><div className="text-right">Fact.</div><div className="text-right">MO</div><div className="text-right">Km</div><div className="text-right">Repuestos</div><div className="text-right">Terceros</div><div className="text-right">Facturación</div><div className="text-right">vs. año ant.</div></div>
