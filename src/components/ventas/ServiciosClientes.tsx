...
export function ServiciosClientes({ desde, hasta, sucursal, buscar, tipoTiempo, marca = "", tipoMaquina = "" }: { desde: string; hasta: string; sucursal: string; buscar: string; tipoTiempo: string; marca?: string; tipoMaquina?: string }) {
  ...
  const grid = "grid-cols-[minmax(220px,1fr)_55px_65px_65px_repeat(4,minmax(92px,1fr))_115px_80px]";
  return <><label className="mt-3 flex items-center gap-2 text-[11px]">Agrupar por<select aria-label="Agrupar clientes por" value={perspective} onChange={e=>setPerspective(e.target.value as "propietario" | "cliente")} className="h-8 rounded-md border bg-background px-2 text-[11px]"><option value="propietario">Propietarios actuales</option><option value="cliente">Clientes facturados</option></select></label><div className="mt-2 overflow-x-auto rounded-md border"><div className="min-w-[1080px]">
    <div className={`grid ${grid} bg-muted/60 px-3 py-2 text-[11px] font-medium text-muted-foreground`}><div>{perspective === "propietario" ? "Cliente" : "Cliente facturado"}</div><div className="text-right">OS</div><div className="text-right">Facturas</div><div className="text-right">Notas de crédito</div><div className="text-right">MO</div><div className="text-right">Km</div><div className="text-right">Repuestos</div><div className="text-right">Terceros</div><div className="text-right">Neto</div><div className="text-right">Participación</div></div>
