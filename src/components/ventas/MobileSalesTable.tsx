...
export function MobileSalesTable<T>({ title, rows, columns, rowKey, sort, toggleSort, primaryKey, metricKey,
  ...
  const renderRow = (row: T, isFooter = false) => <tr key={isFooter ? "total" : rowKey(row)} className={cn("border-t", selected?.(row) && "bg-primary/5", isFooter && "bg-muted/30 font-semibold")}>
    <td className="min-w-0 pl-2 text-left">
      {isFooter ? <span>Total</span> : !onRowClick && primary.render ? <div className="flex min-h-11 min-w-0 items-center overflow-hidden max-sm:whitespace-normal max-sm:break-words max-sm:py-2 max-sm:[&_.truncate]:whitespace-normal max-sm:[&_.truncate]:break-words">{mobileSalesValue(primary, row)}</div> : <button type="button" className="block min-h-11 w-full truncate text-left font-medium underline-offset-4 focus-visible:underline max-sm:whitespace-normal max-sm:break-words max-sm:py-2 max-sm:leading-5" onClick={() => onRowClick ? onRowClick(row) : setDetailKey(rowKey(row))} aria-pressed={onRowClick ? selected?.(row) : undefined}>{mobileSalesValue(primary, row)}</button>}
    </td>
