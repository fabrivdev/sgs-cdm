...
export function SalesDataTable<T>({ title, rows, columns, initialSort, rowKey, fileName, sheetName = "Ventas Servicios",
  ...
  mobileIdentity, mobileEmbedded = false, mobileCountKey, mobileCountLabel, mobilePrimaryLabel, mobileMetricLabel,
}: {
  ...
  mobileCountKey?: string; mobileCountLabel?: string; mobilePrimaryLabel?: string; mobileMetricLabel?: string;
  ...
  if (isMobile) return <><MobileSalesTable embedded={mobileEmbedded} countKey={mobileCountKey} countLabel={mobileCountLabel} primaryLabel={mobilePrimaryLabel} metricLabel={mobileMetricLabel} title={title} rows={ordered} columns={mobileIdentity ? columns.map((c,index)=>index===0?{...c,render:(row:T)=><span className="truncate">{mobileIdentity(row)}</span>}:c) : columns} rowKey={rowKey} sort={sort} toggleSort={toggleSort} footer={footer} onRowClick={onRowClick} selected={selected} empty={empty} />{!mobileCountKey && <RowCount rows={ordered.length} label={countLabel} />}</>;
