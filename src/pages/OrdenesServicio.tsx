...
import { CumplimientoAgendaChart, TecnicosNoRealizadosRanking, MatrizTécnicosDías, EstadoCompacto, CargaSucursalTabla, DistribucionMarca, TrabajosAbiertosList } from "@/components/analytics/OperationalCharts";

const decimal = new Intl.NumberFormat("es-PY", { maximumFractionDigits: 1 });
function Block({ title, children }: { title: string; children: ReactNode }) {
  return <section className="min-w-0 space-y-2 border-t pt-3"><h2 className="text-[13px] font-semibold">{title}</h2>{children}</section>;
}
function JobFollowup({ model }: { model: OperationsModel }) {
  ...
  const columns: CompactListColumn<Row>[] = [
    { key: "ref", label: "OS/TR", kind: "text", width: "w-[15%]", value: r => r.ref, render: r => hasSectionAccess("servicios.trabajos") ? <button type="button" className="min-h-11 text-left text-primary" onClick={() => navigate(`/trabajos?trabajo=${encodeURIComponent(r.id)}`)}>{r.ref}</button> : r.ref },
...
export function OrdersWorkspace() {
  ...
  return <PageShell>
    <PageHeader title="Órdenes de servicio" meta={dates} />
    <Tabs value={tab} onValueChange={setTab} className="min-w-0 space-y-3">
      <TabsList aria-label="Vistas de órdenes de servicio"><TabsTrigger value="ordenes">Órdenes</TabsTrigger><TabsTrigger value="productividad">Productividad</TabsTrigger><TabsTrigger value="cumplimiento">Cumplimiento</TabsTrigger></TabsList>
      <FiltersBar search={{ value: filters.q, onChange: v => change("q", v), placeholder: tab === "cumplimiento" ? "Cliente, trabajo o TR…" : "OS, cliente o chasis…" }} activeCount={active} onClear={() => setFilters(defaults)} secondaryActions={<SalesSectionExportMenu />} expanded={<>
      ...
      {!valid ? <p role="alert" className="text-sm text-destructive">Seleccioná un rango de fechas válido.</p> : query.isError ? <div role="alert" className="space-y-2 text-sm"><p>No se pudieron cargar todas las fuentes. No se muestran resultados parciales.</p><Button variant="outline" size="sm" onClick={() => query.refetch()}>Reintentar</Button></div> : blocked ? <p role="status" className="py-8 text-center text-sm text-muted-foreground">Cargando órdenes y actividad…</p> : <>
        <TabsContent value="ordenes" className="min-w-0 space-y-3">
          <KpiStrip><KpiItem label="Órdenes" value={data.totalOS} /><KpiItem label="Abiertas / sin cierre" value={data.abiertas} tone="info" /><KpiItem label="Cerradas" value={data.cerradas} tone="positive" /></KpiStrip>
        ...
        <TabsContent value="productividad" className="min-w-0 space-y-3">
          <KpiStrip><KpiItem label="Horas-persona" value={decimal.format(data.horasPersona)} /><KpiItem label="Meta disponible" value={data.capacidad.horasDisponibles > 0 ? decimal.format(data.capacidad.horasDisponibles) : "—"} /><KpiItem label="% de meta" value={data.capacidad.horasDisponibles > 0 ? `${decimal.format(data.capacidad.porcentaje)}%` : "—"} /></KpiStrip>
          {query.data?.capacityWarning && <p role="alert" className="text-[12px] text-amber-700">{query.data.capacityWarning}</p>}
        ...
        <TabsContent value="cumplimiento" className="min-w-0 space-y-4">
          <KpiStrip><KpiItem label="Realizadas" value={model.jornadasResultadoResumen.realizadas} tone="positive" /><KpiItem label="No realizadas" value={model.jornadasResultadoResumen.noRealizadas} tone="warning" /><KpiItem label="Pendientes" value={model.jornadasResultadoResumen.pendientes} /></KpiStrip>
          <div className="grid min-w-0 gap-4 lg:grid-cols-2"><Block title="Cumplimiento de agenda"><CumplimientoAgendaChart concise rows={model.cumplimientoAgenda} insights={model.cumplimientoAgendaInsights} /></Block><Block title="No realizadas por técnico"><TecnicosNoRealizadosRanking rows={model.tecnicosNoRealizados} onSelect={id => change("fTécnicos", [id])} /></Block></div>
          {!phone && <Block title="Matriz de técnicos por período"><MatrizTécnicosDías key={`${filters.dateFrom}:${filters.dateTo}:${filters.periodMode}:${model.matrizTécnicosDías.overLimit}:${model.matrizTécnicosDías.blocks.length}`} data={model.matrizTécnicosDías} currentBucketKey={model.matrizTécnicosDías.currentBucketKey} metric={matrixMetric} onMetricChange={setMatrixMetric} onSelectTecnico={id => change("fTécnicos", [id])} onSelectSucursal={s => change("fSucursales", [s])} /></Block>}
