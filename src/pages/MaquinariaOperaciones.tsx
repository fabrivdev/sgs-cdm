      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="pedido" className="space-y-4">
          <DetailSection title="Pedido vinculado" action={canEdit ? <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => onEditHeader(row)}><Pencil className="mr-1.5 h-3 w-3" />{row.np_numero ? "Cambiar" : "Vincular"}</Button> : undefined}>
            {row.np_numero ? <EntityCard><div className="mb-3"><div className="text-[13px] font-semibold">{formatNpCode(row.np_numero)}</div><div className="text-[10px] text-muted-foreground">{row.cliente_nombre || "Cliente no informado"}</div></div><KeyValueGrid className="border-t pt-3"><KeyValueItem label="Comercial" value={row.comercial} /><KeyValueItem label="Fecha NP" value={formatDate(row.np_fecha)} /><KeyValueItem label="Estado del vínculo" value={row.situacion_vinculo} empty="Vinculado" /></KeyValueGrid></EntityCard>
              : (["RESERVADO", "VENDIDO_PENDIENTE_ENTREGA"].includes(row.estado_disponibilidad ?? "") && row.disponibilidad_detalle)
                ? <div className={cn("rounded-lg border p-3", availabilityClass(row.estado_disponibilidad))}><p className="text-[11px] font-medium">Reservada por la venta {row.disponibilidad_detalle}</p><p className="mt-1 text-[10px] opacity-80">Esta máquina se vendió estando en stock; la venta corresponde a ese pedido, pero esta importación no fue registrada específicamente para él. Si en realidad fue importada para esa venta, usá "Vincular" para dejarlo asentado.</p></div>
                : <div className="py-4 text-[11px] text-muted-foreground">Esta máquina todavía no tiene un pedido vinculado.</div>}
          </DetailSection>
        </TabsContent>
