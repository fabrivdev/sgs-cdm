    ...
    <ResponsiveDrawerBody>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="resumen" className="space-y-4">
          <DetailSection title="Unidad" action={canEdit && !editingChassis ? <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => setEditingChassis(true)}><Pencil className="mr-1.5 h-3 w-3" />{row.chasis ? "Cambiar chasis" : "Asignar chasis"}</Button> : undefined}>
        ...
        <TabsContent value="pedido" className="space-y-4">
          <DetailSection title={row.np_numero ? formatNpCode(row.np_numero) : "Sin pedido vinculado"} action={canEdit ? <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => onEditHeader(row)}><Pencil className="mr-1.5 h-3 w-3" />{row.np_numero ? "Cambiar" : "Vincular"}</Button> : undefined}>
            {row.np_numero ? <div className="space-y-3"><div className="text-[12px] font-medium">{row.cliente_nombre || "Cliente sin cargar"}</div><KeyValueGrid><KeyValueItem label="Comercial" value={row.comercial} empty="—" /><KeyValueItem label="Fecha NP" value={formatDate(row.np_fecha)} empty="—" /></KeyValueGrid></div>
  ...
  return <ResponsiveDrawer open={!!operationId} onOpenChange={onOpenChange} size="xl">
    ...
    <ResponsiveDrawerBody>
      {detail && <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="resumen" className="space-y-4">
          <div className={cn("flex flex-wrap items-center gap-x-2 gap-y-0.5 rounded-lg border px-3 py-2.5", nextActionResolved ? "border-emerald-200 bg-emerald-50/60" : "border-amber-200 bg-amber-50/60")}>
            <span className="text-[11px] font-semibold">{nextAction.title}</span>
            <span className="text-[10px] text-muted-foreground">{nextAction.detail}</span>
          </div>
