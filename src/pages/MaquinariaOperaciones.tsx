...
function DeleteDocumentButton({ documentLabel, onDelete, compact = false }: { documentLabel: string; onDelete: () => Promise<void>; compact?: boolean }) {
  ...
  return <AlertDialog>
    <AlertDialogTrigger asChild><Button type="button" variant="ghost" size={compact ? "icon" : "sm"} className={cn("h-8 text-destructive hover:bg-destructive/10 hover:text-destructive", compact && "w-8")} aria-label={compact ? `Eliminar ${documentLabel.toLowerCase()}` : undefined} title={compact ? `Eliminar ${documentLabel.toLowerCase()}` : undefined} disabled={deleting}><Trash2 className={cn("h-3.5 w-3.5", !compact && "mr-1.5")} />{!compact && "Eliminar"}</Button></AlertDialogTrigger>
    <AlertDialogContent>
...
export function ImportDetailDrawer({ row, onOpenChange, onEditHeader, onSaved }: { row: ImportRow | null; onOpenChange: (open: boolean) => void; onEditHeader: (row: ImportRow) => void; onSaved: () => void }) {
    ...
    <ResponsiveDrawerBody>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="resumen" className="space-y-4">
          ...
          <DetailSection card icon={<Ship className="h-3.5 w-3.5" />} title="Pedido y embarque" action={canEdit && !editingImportData ? <Button aria-label="Editar pedido y embarque" variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => { closeEditors(); setEditingImportData(true); }}><Pencil className="mr-1.5 h-3 w-3" />Editar</Button> : undefined}>
        ...
        <TabsContent value="documentos" className="space-y-4">
          ...
          <div className="divide-y">
            <DocumentRow compactActions label="Orden de compra" fileName={ocDocument?.archivo_nombre} date={ocDocument ? formatDate(ocDocument.creado_en) : null} onOpen={ocDocument ? () => openMachineDocument(ocDocument.storage_path, ocDocument.archivo_nombre) : undefined} action={canEdit ? <><Button variant="ghost" size="icon" className="h-8 w-8" aria-label={ocDocument ? "Reemplazar orden de compra" : "Adjuntar orden de compra"} title={ocDocument ? "Reemplazar orden de compra" : "Adjuntar orden de compra"} disabled={uploadingOc} onClick={() => detailOcRef.current?.click()}><Upload className="h-3.5 w-3.5" /></Button>{ocDocument && <DeleteDocumentButton compact documentLabel="Orden de compra" onDelete={async () => { await deleteMachineDocuments({ importLineId: row.importacion_linea_id, type: "OC" }); await detailOcQuery.refetch(); }} />}</> : undefined} />
            <DocumentRow compactActions label="Factura del proveedor" fileName={supplierDocument?.archivo_nombre} date={supplierDocument ? formatDate(supplierDocument.creado_en) : null} onOpen={supplierDocument ? () => openMachineDocument(supplierDocument.storage_path, supplierDocument.archivo_nombre) : undefined} action={canEdit ? <><Button variant="ghost" size="icon" className="h-8 w-8" aria-label={supplierDocument ? "Reemplazar factura del proveedor" : "Adjuntar factura del proveedor"} title={supplierDocument ? "Reemplazar factura del proveedor" : "Adjuntar factura del proveedor"} disabled={uploadingSupplierInvoice} onClick={() => detailSupplierInvoiceRef.current?.click()}><Upload className="h-3.5 w-3.5" /></Button>{supplierDocument && <DeleteDocumentButton compact documentLabel="Factura del proveedor" onDelete={async () => { await deleteMachineDocuments({ importLineId: row.importacion_linea_id, type: "FACTURA_IMPORTACION" }); await detailSupplierInvoiceQuery.refetch(); }} />}</> : undefined} />
          </div>
          <DetailSection card icon={<FileText className="h-3.5 w-3.5" />} title="Factura proveedor" help="Importe facturado por el proveedor para esta unidad, separado del costo de stock." action={canEdit && !editingInvoice ? <Button aria-label="Editar factura proveedor" title="Editar factura proveedor" variant="ghost" size="icon" className="h-7 w-7" onClick={() => { closeEditors(); setEditingInvoice(true); }}><Pencil className="h-3.5 w-3.5" /></Button> : undefined}>
            {editingInvoice ? <ImportUnitFields section="invoice" form={form} onChange={setForm} onCancel={cancelEdit} onSave={() => saveUnit("invoice")} saving={saving} /> : hasSupplierInvoiceData ? <KeyValueGrid><KeyValueItem label="Número" value={row.invoice_supplier} empty="—" mono /><KeyValueItem label="Fecha" value={formatDate(row.factura_proveedor_fecha)} empty="—" /><KeyValueItem label="Moneda" value={row.factura_proveedor_moneda} empty="—" /><KeyValueItem label="Valor factura" value={row.valor_factura_proveedor == null ? null : formatImportMoney(row.valor_factura_proveedor, row.factura_proveedor_moneda ?? "USD")} empty="Sin cargar" /></KeyValueGrid> : <p className="text-[11px] text-muted-foreground">Sin datos cargados</p>}
          </DetailSection>
        </TabsContent>
      ...
      : simpleState === "COMPLETADO"
        ...
        : missingOriginCount > 0
          ...
          : missingAgreedValueCount > 0
            ...
            : billingComplete
              ? { title: "Facturación completa", detail: deliveryComplete ? "Entrega confirmada." : "Falta confirmar la entrega." }
              : { title: "Facturación pendiente", detail: `${unitBillingCount}/${totalUnits} unidades confirmadas.` };
  const nextActionResolved = ["Pedido cancelado", "Proceso completo", "Facturación completa"].includes(nextAction.title);
    ...
    <ResponsiveDrawerBody>
      {detail && <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid h-auto w-full grid-cols-3">
          ...
          <TabsTrigger value="trazabilidad" className="px-2 text-[11px]">Origen</TabsTrigger>
          <TabsTrigger value="documentos" className="px-2 text-[11px]">Documentos</TabsTrigger>
        </TabsList>

        <TabsContent value="resumen" className="space-y-4">
          <div className={cn("flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5", nextActionResolved ? "border-emerald-200 bg-emerald-50/60" : "border-amber-200 bg-amber-50/60")}>
          ...
          <DetailSection card icon={<FileCheck2 className="h-3.5 w-3.5" />} title="Información general">
            <KeyValueGrid className={cn(detail.lines.length > 1 ? "sm:grid-cols-4" : "sm:grid-cols-3")}>
              ...
              <DetailValue label="Comercial" value={shortPersonName(detail.comercial)} />
          ...
          <DetailSection card icon={<FileText className="h-3.5 w-3.5" />} title="Facturación">{invoices.length ? <div className="divide-y">{invoices.map((invoice, index) => <KeyValueGrid key={`${invoice.numero}-${invoice.fecha}-${index}`} className="py-3 first:pt-0"><KeyValueItem label="Valor" value={formatMoney(invoice.valor, invoice.moneda)} empty="—" /><KeyValueItem label="Número" value={invoice.numero} empty="—" mono /><KeyValueItem label="Fecha" value={invoice.fecha ? formatDate(invoice.fecha) : null} empty="—" /></KeyValueGrid>)}</div> : <div className="rounded-lg bg-muted/40 px-3 py-2.5 text-[11px] text-muted-foreground">Sin factura confirmada</div>}</DetailSection>
          ...
          {detail.observaciones && <DetailSection card icon={<FileText className="h-3.5 w-3.5" />} title="Observaciones"><div className="text-[11px] leading-relaxed">{detail.observaciones}</div></DetailSection>}
        </TabsContent>
