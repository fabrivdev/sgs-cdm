...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      supabase
        .from("servicios")
        .select("id, fecha_programada, tecnico_responsable_id, auxiliares, sucursal, marca, estado, horas_trabajadas"),
  ...
  const porSucursal = SUCURSALES.map((s) => ({
    name: s,
    total: filtered.filter((x) => x.sucursal === s).length,
  }));
  ...
  /*
    IMPORTANTE:
    Antes este cálculo contaba solamente al tecnico_responsable_id.
    Ahora cuenta:
    - responsable
    - auxiliares
    Un mismo servicio suma 1 servicio a cada técnico participante.
    Las horas trabajadas también se suman a cada participante.
  */
  const porTecnico = useMemo(() => {
    ...
    for (const s of filtered) {
      ...
      for (const aux of s.auxiliares ?? []) {
        if (aux) participantes.add(aux);
      }
      ...
      for (const id of participantes) {
        ...
        const actual =
          ...
          {
            id,
            name: shortName(profById[id] ?? "Sin nombre"),
            total: 0,
            horas: 0,
          };
    ...
    return [...base.values()].sort((a, b) => b.total - a.total || b.horas - a.horas);
        ...
        <Card className="p-4">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart margin={{ top: 16, right: 16, bottom: 16, left: 16 }}>
              ...
              >
                {porMarca.map((e) => (
                  <Cell key={e.name} fill={COLORS_MARCA[e.name as Marca]} />
                ))}
              </Pie>
              <Legend />
              <Tooltip />
