...
## Follow-up — reconciled invoice amounts and OS efficiency, 25/09/2026

The user confirmed that the hourly rate is per OS working hour, not per technician. Added a fourth Productivity KPI using the existing KpiStrip, beside the unchanged capacity/productivity measures. No new tab, chart, global style or permanent explanatory paragraph. The financial drawer retains its existing three groups, shared brand badges and money formatting; billed-equivalent hours and efficiency occupy the existing compact key/value grid. Unknown financial values are dashes rather than operational amounts or invented zeros.

Visual inspection used the ignored synthetic harness with real components at 320×780. The four KPIs form two rows of two; the drawer shows hours worked, hours billed and efficiency separately, with the total below. Document scrollWidth/clientWidth both 305 px excluding scrollbar. At 768×860 both document widths were 768 px. An initial 1280×860 screenshot showed the four-card desktop strip during a transition; the later screenshot attempt timed out, so desktop/tablet are not claimed as fully settled screenshot verification. Component integration tests cover 320/768/1280, and existing responsive regressions cover six breakpoints. No physical device or production invoice verification.

144 tests passed in 11 files, including 18 financial loader/calculation cases and 37 OS page cases; ES2021 types, targeted lint and production build passed. Existing React Router future/chunk-size warnings remain. PostgreSQL-embedded fixture executes the new read-only migration twice and verifies multiple invoice rates, NC signs, exact branch-prefixed OS identities, date cutoffs/quarter boundaries, ambiguity, missing rates and restricted access. Its canonical Sales function is a fixture, not the production ledger. Financial access requires both OS and Sales permissions; migration still requires manual Cloud application. No imported records, Sales UI, productivity goal, existing days KPI or broad access policy was changed. Updated note 26; this checkout has no configured Obsidian synchronization.

## Follow-up — non-blocking financial loading, 25/09/2026

The first financial loader delayed the entire operational snapshot and queried the one-year lookback instead of only the displayed OS population. Financial reconciliation now has an independent query/cache, explicit retry and 25-second deadline. It leaves operational tables, KPIs and navigation available; only efficiency and financial drawer values wait. The order export stays disabled during reconciliation. The drawer resolves its selected identity against current rows so it updates when the response arrives. No layout redesign or permanent explanatory copy.

Browser evidence: real section components in the ignored localhost fixture, with `?slow-billing` deliberately keeping finance pending. Orders and its 14 synthetic rows remained visible. Desktop screenshot at the default 1280×720 shows productivity hours/target/progress and its tables while only Efficiency says Calculando. A settled 320×780 screenshot shows the same separation, compact four-card composition and technician rows. Clicking Cumplimiento during that pending state displays its operational tables and KPIs. Viewport override reset afterward. This is a controlled pending-state test, not a production timing measurement or network benchmark.

153 tests passed in 12 files, including 20 billing cases, four real-query-hook cases and 40 page cases. Targeted lint, ES2021 types and production build passed; existing router/chunk-size warnings remain. The isolated PostgreSQL script verifies equivalence with the actual current-system Sales classifier, two idempotent migrations, exact indexed OS lookup with 25,000 unrelated synthetic invoice lines, rates, credits, cutoff, historical links, ambiguity and permission checks. Historical resolution remains a fixture. The optimized SQL requires manual Cloud application; no production SQL, imported-row changes or permission expansion performed. Note 26 updated; no Obsidian synchronization claimed.

## Follow-up — productivity by recorded work dates, 25/09/2026

Productivity now reads actual clock intervals independently of the OS opening/closing cohort. Same existing four-card strip, compact status selector, progress bars and tables. The OS drawer gains a shared DetailSection for dated work; clicking a technician opens the period's actual work records rather than the lifecycle-selected OS list. Desktop shows date/time/technician/hours, phone groups date/time/identity/context with hours right. No explanatory paragraph or new accordion. Incomplete/invalid work is explicitly inspectable, percentages remain unknown and productivity exports are disabled; other OS information stays usable.

Browser: ignored localhost harness with real components, synthetic work in August and September. At 1280×900, Productivity counts September only; OS detail retains August plus September, while technician drill-down keeps the selected month. Inspected mobile drawer screenshots at 390×844 and 320×740; date, clock interval, hours and inherited participation remain readable. Document clientWidth/scrollWidth match at 390, 320 and 1280. Initial resize captures were stale; later settled captures were used. Desktop drawer review detected a clipped clock/header; adjusted column widths and compact clock separator. No physical device, production timing or real record validation claimed.

180 tests pass across 14 files, including real work query laziness, cancellation/user/range isolation, pagination, deadline, late failure, opening/closing independence, overnight splits, duplicate/overlapping clocks, filters, missing detail, full OS history and existing financial/closure/Sales regressions. Targeted lint, ES2021 types and build passed. Embedded PostgreSQL verifies idempotent SQL and restricted read-only execution with fixtures; no Cloud SQL applied. Manual migration `20260925230000_service_order_work_log.sql` is required for activation. Note 26 documents source coverage and the unchanged capacity/efficiency rules; no Obsidian synchronization claimed.
