-- Corrige la definicion de "vinculada_os" en ventas_area_resumen.
--
-- Bug: vinculada_os usaba origen_sistema = 'new_xml_facturacion_os' como proxy de
-- "esta atada a una orden de servicio real". Ese origen_sistema se pone para
-- CUALQUIER match del crosswalk (document, invoice, product_code o
-- manufacturer_code) -- no solo para los que matchean una OS real (document/
-- invoice, que son los unicos que dejan linked_service_order no nulo). Por eso,
-- ventas de repuestos de mostrador que matchean el catalogo de productos por
-- codigo (sin ninguna OS de por medio) quedaban con vinculada_os = true y caian
-- en el area 'servicios' en vez de 'repuestos'.
--
-- Fix: vinculada_os pasa a depender unicamente de linked_service_order IS NOT
-- NULL, que es el campo que de verdad solo se llena cuando el crosswalk
-- encontro una orden de servicio (matchedBy = 'document' | 'invoice').
--
-- Efecto esperado en el CASE de area (sin cambios en su estructura):
--   - Repuestos con OS real (linked_service_order no nulo) -> siguen en 'servicios'.
--   - Repuestos de mostrador (linked_service_order nulo) -> vuelven a 'repuestos'.
--   - Servicio/Kilometraje -> sin cambio (ya caian en 'servicios' por concepto,
--     no dependian de vinculada_os).
--   - Maquinarias -> sin cambio (primera rama del CASE, no depende de vinculada_os).
