...
WITH stock AS (
  ...
  SELECT
    ...
    coalesce(m.codigo_fabricante, sv.codigo_fabricante, s.codigo_fabricante) AS codigo_fabricante,
    'CLAAS'::text AS marca,
    coalesce(s.stock, 0)::numeric AS stock,
...
COMMENT ON VIEW public.v_repuestos_reporte_claas IS
  'Reporte especifico CLAAS de codigo, fabricante, stock y ventas; la marca historica se deriva del subgrupo.';
