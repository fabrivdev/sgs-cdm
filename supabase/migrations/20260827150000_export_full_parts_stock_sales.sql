-- Base completa para exportar stock y ventas de repuestos.
-- El universo no nace del stock: incluye catalogo, codigos presentes solo en
-- stock, maestro anterior y ventas historicas aun no vinculadas.
...
WITH stock AS (
  ...
  SELECT
    ...
    nullif(trim(f.codigo_fabricante), '') AS codigo_fabricante,
  ...
  SELECT
    ...
    CASE WHEN p.grupo = 'HISTORICO' THEN 'HISTORICO' ELSE 'CATALOGO' END AS origen,
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM maestro_anterior m WHERE m.codigo_legacy_norm = vs.codigo_norm
  )
    AND NOT EXISTS (
...
COMMENT ON VIEW public.v_repuestos_stock_ventas_exportacion IS
  'Universo completo de repuestos para exportacion: catalogo, stock cero, maestro anterior y ventas historicas no vinculadas.';
