-- Corrige el reporte multimarcas: excluye mercaderias del maestro que no son
-- repuestos y consolida cada codigo historico en una sola fila.

CREATE OR REPLACE FUNCTION public.repuestos_es_subgrupo_legacy_repuesto(p_subgrupo text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $function$
  SELECT
    upper(trim(coalesce(p_subgrupo, ''))) IN ('SERVICE - CLAAS', 'SERVICE - HORSCH')
    OR coalesce(p_subgrupo, '') ~* 'repuest|parts';
$function$;
...
WITH stock AS (
  ...
  WHERE c.activo
    ...
    AND public.repuestos_es_subgrupo_legacy_repuesto(m.tipo)
  ...
  SELECT
    ...
    CASE
      WHEN bool_or(b.marca = 'CLAAS') THEN 'CLAAS'
      WHEN bool_or(b.marca = 'HORSCH') THEN 'HORSCH'
      ELSE 'OTROS'
    END AS marca,
...
), universo_deduplicado AS (
  SELECT DISTINCT ON (upper(trim(u.codigo))) u.*
  FROM universo u
  WHERE NOT (
    u.origen IN ('MAESTRO_ANTERIOR', 'VENTA_HISTORICA')
    AND nullif(trim(u.codigo_fabricante), '') IS NULL
    AND coalesce(u.stock_total, 0) = 0
    AND coalesce(u.ventas_36m, 0) = 0
  )
  ORDER BY
    upper(trim(u.codigo)),
    CASE u.origen
      WHEN 'CATALOGO_MIXTO' THEN 1
      WHEN 'CATALOGO' THEN 2
      WHEN 'HISTORICO' THEN 3
      WHEN 'MAESTRO_ANTERIOR' THEN 4
      WHEN 'VENTA_HISTORICA' THEN 5
      ELSE 6
    END,
    CASE u.marca WHEN 'CLAAS' THEN 1 WHEN 'HORSCH' THEN 2 ELSE 3 END,
    (abs(u.ventas_36m) + abs(u.stock_total)) DESC,
    u.codigo
)
SELECT
  u.*,
  current_date AS fecha_corte
FROM universo_deduplicado u;
...
COMMENT ON VIEW public.v_repuestos_stock_ventas_exportacion IS
  'Reporte completo de repuestos, sin duplicados historicos y excluyendo subgrupos no relacionados con repuestos.';
