...
-- Regla confirmada para el histórico: una OS sin tipo de tiempo, atribuida a
-- un cliente externo, es Cliente. CAMPOS DEL MAÑANA y nombres ausentes quedan
-- pendientes de confirmación. No modifica la OS ni las jornadas originales.
CREATE OR REPLACE FUNCTION public.ventas_tipo_tiempo_historico_cliente(
  p_cliente text, p_tipo text DEFAULT NULL
)
...
SET search_path=public,pg_temp AS $$
  ...
  FROM claves;
$$;
REVOKE ALL ON FUNCTION public.ventas_tipo_tiempo_historico_cliente(text,text)
  FROM PUBLIC,anon,authenticated;

-- El historial completo necesita los importes originales de cada componente.
-- La clasificación automática se limita al tramo legado; el sistema nuevo
-- conserva exclusivamente el tipo informado en su archivo.
    ...
    INTO result FROM (
      SELECT os_numero, fecha_abierta_os, fecha_cierre_os,
        coalesce(
          ...
          THEN public.ventas_tipo_tiempo_historico_cliente(cliente_nombre,tipo_tiempo)
          END,
          tipo_tiempo
        ) AS tipo_tiempo,
