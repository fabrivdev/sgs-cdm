-- Removes repeated scalar lookups from all three service RPCs.
-- UUID-to-UUID join preserves the billing primary-key index and cardinality.
BEGIN;
...
returns text language sql stable set search_path = public, pg_temp as $$
  select coalesce(
    (select public.ventas_tipo_tiempo_normalizado(coalesce(nullif(f.tipo_tiempo, ''), nullif(f.raw_data->>'canonical_time_type', ''), p_os_tipo))
     ...
     where p_metodologia = 'actual' and f.id = p_id::uuid),
...
begin
  ...
  with base as materialized (
    select b.*, public.ventas_tipo_tiempo_normalizado(coalesce(nullif(fl.tipo_tiempo, ''), nullif(fl.raw_data->>'canonical_time_type', ''), os.tipo_tiempo)) as tipo_tiempo
    from public.ventas_area_movimientos_base(p_desde, p_hasta, p_sucursal, null) b
    left join public.facturacion_lineas_importadas fl
      on fl.id = case when b.metodologia = 'actual' then b.linea_id::uuid else null::uuid end
    left join public.ordenes_servicio_importadas os
