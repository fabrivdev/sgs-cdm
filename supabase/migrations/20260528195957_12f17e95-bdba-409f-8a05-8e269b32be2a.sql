...
-- 1) Backfill sucursal en servicios legacy desde el trabajo vinculado
UPDATE public.servicios s
SET sucursal = t.sucursal
FROM public.trabajos t
WHERE t.legacy_servicio_id = s.id
  AND s.sucursal IS NULL
  AND t.sucursal IS NOT NULL;

-- 2) RPC para programar jornada saneando el servicio asociado
...
BEGIN
  ...
  IF NOT (
    ...
    OR (public.has_role(v_uid, 'cabecilla'::public.app_role)
